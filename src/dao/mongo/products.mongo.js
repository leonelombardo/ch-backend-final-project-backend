...
class ProductsDAO{
    ...
    async deleteOne(id, user){
        ...
        if(user.role === "premium" && user.email === product.owner){
            const message = `Your product <span style="font-weight: bold;">${product.title}</span> <span style="font-weight: bold;">(${product.id})</span> was successfully deleted.`;
            
            await sendEmail(user.email, `Product deleted`, message);

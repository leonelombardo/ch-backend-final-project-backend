...
class CartsDAO{
    ...
    async findById(id){
        ...
        const response_products = await fs.promises.readFile(this.products);
        const products = JSON.parse(response_products);
      
        const products_data = [];
        
        cart.products.forEach(product => products.find(x => x.id === product.product && products_data.push({ ...product, product: x })));

        return { ...cart, products: products_data };
    }
